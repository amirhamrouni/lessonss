...
function RouteFallback(){return <div className="app-shell"><div className="phone"><main className="page"><section className="mode-empty route-loading" aria-live="polite"><span className="eyebrow">ENGLISH TWIN</span><h2>Loading your learning space…</h2><p>Your saved progress stays in place while this screen opens.</p></section></main></div></div>;}

function Root(){return <AppErrorBoundary><Suspense fallback={<RouteFallback/>}><Routes><Route path="/welcome" element={<AuthGateway/>}/><Route path="/" element={<SmartHomeV2/>}/><Route path="/setup" element={<LearnerSetup/>}/><Route path="/start" element={<BeginnerFoundation/>}/><Route path="/learn" element={<LearnJourney/>}/><Route path="/lesson/:lessonId" element={<LessonRouter/>}/><Route path="/practice" element={<PracticeHub/>}/><Route path="/review" element={<ReviewMode/>}/><Route path="/sentence-builder" element={<AdaptiveSentenceBuilder/>}/><Route path="/assessment" element={<AssessmentV2/>}/><Route path="/checkpoint" element={<LevelCheckpoint/>}/><Route path="/twin" element={<TutorMode/>}/><Route path="/speak" element={<SpeechDrill/>}/><Route path="/pronunciation" element={<PronunciationGateway/>}/><Route path="/speak/live" element={<VoiceLab/>}/><Route path="/profile" element={<ProfileHub/>}/><Route path="/mistakes" element={<MistakeMemory/>}/><Route path="/privacy" element={<PrivacyPolicy/>}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes></Suspense></AppErrorBoundary>;}

const routerBase=import.meta.env.BASE_URL.replace(/\/$/,'')||'/';
ReactDOM.createRoot(document.getElementById('root')!).render(<React.StrictMode><BrowserRouter basename={routerBase}><Root/></BrowserRouter></React.StrictMode>);
