...
export default function SmartHomeV2(){
  ...
  },[profile?.placementLevel,dailyPlan,dueCount,nextLesson]);
  ...
  return <div className="app-shell"><div className="phone"><main className="page">
    ...
    <section><div className="section-heading"><span>TODAY</span><h3>Your adaptive plan</h3></div><div className="daily-plan">{dailyPlan.map((item,index)=><button key={`${item.id}-${index}`} onClick={()=>nav(planRoute(item.id,item.lessonId))}><span className="plan-no">{String(index+1).padStart(2,'0')}</span><div><b>{item.id==='review'?'Memory review':item.id==='lesson'?'Next lesson':item.id==='weak-skill'?`Strengthen ${item.skill}`:'Speaking transfer'}</b><small>{item.minutes} min · {item.reason}</small></div><ChevronRight/></button>)}</div></section>
    ...
    <section><div className="section-heading"><span>SIGNALS</span><h3>Why this plan</h3></div><div className="daily-plan"><button onClick={()=>nav('/assessment')}><span className="plan-no">01</span><div><b>Placement</b><small>{profile.placementLevel?`Measured: ${profile.placementLevel}`:'Not measured yet'}</small></div><ChevronRight/></button><button onClick={()=>nav('/review')}><span className="plan-no">02</span><div><b>Memory</b><small>{dueCount?`${dueCount} cards due`:'No review due'}</small></div><ChevronRight/></button><button onClick={()=>nav('/practice')}><span className="plan-no">03</span><div><b>Weakest skill</b><small>{weakestSkill||'Not enough evidence yet'}</small></div><ChevronRight/></button></div></section>
