...
function renderActivity(
  ...
  if (activity.type === 'explain') {
    return <div className="explain"><span className="eyebrow" dir={dir}>{t(supportLanguage, 'learn')}</span><div className="native-instruction" dir={dir}>{t(supportLanguage, 'nativeQuestion')}</div><h2>{activity.title}</h2><p>{activity.body}</p><div className="examples">{activity.examples.map(example => <div key={example}>{example}</div>)}</div></div>;
  }
  if (activity.type === 'choice') {
    return <div><span className="eyebrow" dir={dir}>{t(supportLanguage, 'choose')}</span><div className="native-instruction" dir={dir}>{t(supportLanguage, 'target')}</div><h2>{activity.prompt}</h2><div className="answer-list">{activity.options.map(option => <button disabled={locked} className={selected === option ? 'selected' : ''} key={option} onClick={() => setSelected(option)}>{option}</button>)}</div></div>;
  }
...
export default function AutoLessonPlayer() {
  ...
  function continueExplanation() {
    if (index >= currentLesson.activities.length - 1) {
      void finish(correct, total);
      return;
    }
    goNext();
  }
  ...
  function check() {
    ...
    timerRef.current = setTimeout(() => {
      if (isLast) void finish(nextCorrect, nextTotal);
      else goNext();
    }, 950);
  }
  ...
  return <Frame>
    ...
    <section className="activity-card">
      ...
      <button className="primary activity-action" disabled={saving || transitioning || (activity.type !== 'explain' && !(selected || fill.trim()))} onClick={check}>
        {saving ? 'Saving…' : transitioning ? (index >= currentLesson.activities.length - 1 ? 'Finishing…' : 'Next question…') : activity.type === 'explain' ? t(supportLanguage, 'continue') : t(supportLanguage, 'check')}
      </button>
    </section>
  </Frame>;
}
