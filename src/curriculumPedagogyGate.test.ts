...
import { b1, b2, c1, lessonsForLevel, unitsForLevel } from './curriculumAll';
import { isLessonV2 } from './curriculumV2';

const advanced = { B1: b1, B2: b2, C1: c1 } as const;

function unitLessons(level: keyof typeof advanced, unitId: string) {
  return lessonsForLevel(level).filter(lesson => lesson.unitId === unitId && isLessonV2(lesson));
}
...
describe('advanced curriculum pedagogical release gate', () => {
  ...
  it('requires multi-source synthesis in every B2 and C1 unit', () => {
    for (const level of ['B2','C1'] as const) {
      for (const unit of unitsForLevel(level)) {
        const synthesis = unitLessons(level, unit.id).flatMap(lesson => lesson.activities).filter(activity => activity.type === 'paragraph_synthesis');
